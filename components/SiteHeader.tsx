  ...
  return (
    <header
      className={[
        ...
        scrolled ? "border-[var(--border)] bg-[var(--bg)]/90 backdrop-blur-md" : "border-transparent bg-transparent",
      ].join(" ")}
    >
      <div className="layout-shell flex h-14 items-center justify-between gap-4">
        <button type="button" onClick={() => go("home")} className="focus-ring text-sm font-semibold tracking-tight">
          {site.name.split(" ")[0]}
            ...
            <nav className="layout-shell space-y-0.5 py-3" aria-label="Mobile sections">
              {nav.map((item) => (
                <button
                  ...
                  className={[
                    "w-full rounded-md px-2 py-2.5 text-left text-sm font-medium",
                    active === item.id ? "text-[var(--text)]" : "text-[var(--text-secondary)]",
                  ].join(" ")}
                >
                  {item.label}
                </button>
